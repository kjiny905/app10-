...
//    uploadFile.setRid(rid);
    ...
    uploadFile.setFsize(String.valueOf(mf.getSize()));  //파일size를 문자varchar2로 저장.연산이 필요없이 정보로서 문자열로 저장.
  ...
  private String extractExt(String originalFile) {
    int posOfExt =originalFile.lastIndexOf(".");  //.다음 확장자
    String ext = originalFile.substring(posOfExt + 1);
    return ext; //확장자만 추출 ex) **.png
  }
}
